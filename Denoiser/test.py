...
import argparse
import time
from utils import calculate_mrse, calculate_psnr, calculate_ssim, tonemap, save_output_exr
from model import *
from dataset import *
from utils import *
import settings
...
def main():
	...
	total_time = 0.
	with torch.no_grad():
		for i, item in enumerate(testing_generator):
			for k in item.keys():
				item[k] = item[k].float().cuda()

			inp = item['A']
			gt = item['B']
			albedo = item['ALBEDO'][:, 0, 0:3, ...]
			gt_albedo = item['ALBEDO'][:, 0, 3:6, ...]
	...
	logger.info('[Test {}] psnr: {:.4f} ssim: {:.4f} rmse: {:.4f} average time: {:.4f}'.format(
	...
	os.system("ffmpeg -r 30 -apply_trc iec61966_2_1 -i {} {}".format(
