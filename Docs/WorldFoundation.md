...
- Flecs 已在 ThirdParty 中，但尚未接入第一方 Runtime。
...
- **hot reload 分两类，不能一概而论"RenderScene 无 dirty"**：
  - **PayloadCompatible**（layout 与 bounds 不变，只替换纹理像素、材质常量等）——只更新间接表里该 ID 指向的 payload version，RenderScene 不用 dirty。旧 payload 在最后一个引用它的 frame fence 完成后由 Registry 销毁。
...
Asset record 自身在 Ready 生命周期内持有对**它的依赖**的 strong dependency lease；payload 只保存 `AssetID` 或已解析的稳定 `GpuMeshID`/`GpuMaterialID`，不保存依赖 GPU 指针。依赖的 hot reload **不**自动重载父资产，除非父 loader 显式声明"依赖版本会影响自身 payload"。
...
- **子资源身份分两类**：submesh 使用 `MeshAsset` 内部的稳定 `SubmeshID`（不是全局 `AssetID`），material 是独立资产，使用真正的 `AssetID`。首次导入时分配 `SubmeshID` 并写入 sidecar；重新导入优先使用 exporter/source 提供的持久身份，几何、材质和名称签名只作为匹配提示，不能直接作为身份。无法唯一匹配时分配新 ID 并报告，不能静默把已有 material override 绑定到另一个 submesh。
...
- **array-of-struct**：元素是固定 schema、可平凡移动的小结构体，字段仍由上述标量/ID property 描述。第一阶段所有 reflected dynamic array 使用统一的 Scene array header（data、count、capacity）和 Scene allocator；metadata 提供 element descriptor，并通过统一 operations 完成 count/data 查询和 resize/insert/remove。Serializer、Inspector 和 undo/redo 不直接缓存 data pointer，所有 mutation 都经过 World property API，以便产生 dirty/change notification。
...
- **RenderObject** 持 mesh 级共享数据（material overrides、instance range），一个 Entity 一份。
...
- current/previous transform 在明确的帧边界推进，且必须处理"移动后一帧不再 dirty"的 motion vector 失真：对象在第 N 帧移动（previous=old、current=new）后，第 N+1 帧即使不再 dirty 也要把 GPU 侧 previous 推进为 current，否则 previous 会停在 old、motion vector 连续多帧错误。采用以下之一（第一版取 history-dirty 列表，最省带宽）：①变化后保持**两帧 dirty**，下一帧上传 previous=current；②**frame-indexed transform snapshot/buffer**（读上一帧 buffer 作 previous）；③单独维护一个 **history-dirty 列表**，只补推进这些对象的 previous。
...
1. **instance 层已是一等公民（不是半预留）**：三层模型里 transform 就在 `RenderInstance`，`RenderObject` 持 instance range。phase 1 每 object 恰好 1 instance，但 buffer layout 已按"object → instance range → RenderInstance[]"建好，将来 ISM/foliage 只是让某些 object 的 instance range > 1，**不改 layout**。对应 UE GPUScene 的 `FPrimitiveSceneData`（≈RenderObject）+ `FInstanceSceneData`（≈RenderInstance）。
...
- GPU resource retirement 的所有权明确划分：Asset Registry 独占 Mesh/Texture/Material payload 和 GPU table snapshots；RenderScene 独占自身 object/instance/draw/light/camera buffers 和 snapshots。`SceneFrameSnapshot` 只持有两侧 snapshot leases，不取得底层资源所有权。Registry 只在没有长期 asset lease、没有 snapshot 引用且相关 GPU fence 完成后销毁 payload。
...
5. 版本化 cooked SceneAsset schema，并更新 AssetPipeline 输出 node tree、局部变换、MeshAsset/submesh/default-material 表和持久 `SubmeshID`；旧 schema 在新 World 路径中明确要求 re-cook，迁移期按需双写仅供旧 Renderer 使用的 compatibility chunk。
...
8. 实现 RenderScene 三层 slot（RenderObject/RenderInstance/RenderDraw）、resolve/binding 层（把 `MeshRenderer.AssetID`/`SubmeshID` 解析为 `GpuSubmeshRef`/`GpuMaterialID` 并持有长期 asset lease）、dirty ranges、配对 snapshot publish 和 frame-safe retirement。
...
- **可延后到 phase-1.5**：
  ...
  - **hot reload 完整实现**（间接表 + version 的 contract 现在就留，触发重载与新旧 version 切换延后）。
...
- Cooker tests：node hierarchy/local matrix round trip、primitive reorder 后 `SubmeshID` 保持、歧义匹配产生新 ID 和诊断，以及旧 cooked schema 返回 re-cook 错误。
- Asset tests：重复请求、dependency cycle、每个加载阶段的故障注入、cancellation、迟到完成消息、loading 期间 release、长期 lease 随 MeshRenderer 删除而释放、三类 GpuID 的 stale handle，以及 PayloadCompatible/StructuralChange hot reload。
...
- Transform tests：深层 hierarchy、cycle rejection、reparent、parent deletion、**奇异 parent 下 `setParent(keepWorld)` 返回失败且 parent 不变**、`LocalTransform`/`LocalMatrix` 互斥，以及 **previous transform 在移动后 N+1 帧仍被正确推进（motion vector 不连续出错）**。
...
- RenderScene tests：三层 slot reuse、一个多-submesh Entity 展开成多个 RenderDraw、StructuralChange 原子切换配对 snapshots、dirty range、asset-not-ready、entity removal 和无逐帧分配/逐 draw lease。
...
- Cooked SceneAsset 保存可编辑 node hierarchy；正式 World 路径不依赖旧的 flat `SceneAssetInstance` 适配。
...
- Render extraction 正常帧无内存分配，只上传 dirty ranges。
