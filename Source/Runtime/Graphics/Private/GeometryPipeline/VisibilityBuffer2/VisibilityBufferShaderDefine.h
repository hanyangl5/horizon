...
#define GET_ALPHA_OUTPUT_VB_ID(triId, batchId) \
    ((1 << ALPH_IS_LOW_BIT) | (((triId) << TRI_ID_LOW_BIT) & TRI_ID_MASK) | (((batchId) << BATCH_ID_LOW_BIT) & BATCH_ID_MASK))

#define PACK_BATCH_DATA(geomSet, triId, materialId)                                 \
    (((geomSet) << ALPH_IS_LOW_BIT) | (((triId) << TRI_ID_LOW_BIT) & TRI_ID_MASK) | \
     (((materialId) << MATERIAL_ID_LOW_BIT) & MATERIAL_ID_MASK))
...
#define MAX_PRIMITIVES_PER_DRAW_INDIRECT \
    (1 + (BATCH_ID_MASK >> BATCH_ID_LOW_BIT)) // Maximum value that can be represented with # bits PrimitiveID
#endif
