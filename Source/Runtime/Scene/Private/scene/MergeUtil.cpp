...
{
    // Find material index
    const int oldMaterial =
        (int)std::distance(std::begin(scene.materialNames),
                           std::find(std::begin(scene.materialNames), std::end(scene.materialNames), materialName));

    std::vector<uint32_t> toDelete;

    for (size_t i = 0u; i < scene.hierarchy.size(); i++)
    {
        if (scene.meshForNode.contains(i) && scene.materialForNode.contains(i) &&
    ...
    // TODO: if merged mesh transforms are non-zero, then we should pre-transform individual mesh vertices in meshData
...
void mergeMaterialLists(const std::vector<std::vector<Material> *> &oldMaterials,
                        const std::vector<std::vector<std::string> *> &oldTextures, std::vector<Material> &allMaterials,
