...
/**
  ...
  The simplest one is the direct "gluing" of multiple scenes into one [all the material lists and mesh lists are merged
  and indices in all scene nodes are shifted appropriately] The second one is creating a "grid" of objects (or scenes)
...
void deleteSceneNodes(Scene &scene, const std::vector<uint32_t> &nodesToDelete)
{
    ...
    // 1.a) Move all the indicesToDelete to the end of 'nodes' array (and cut them off, a variation of swap'n'pop for
    // multiple elements)
    ...
    // 4) As in mergeScenes() routine we also have to adjust all the "components" (i.e., meshes, materials, names and
    // transformations)
    ...
    // 5) scene node names list is not modified, but in principle it can be (remove all non-used items and adjust the
    // nameForNode_ map) 6) Material names list is not modified also, but if some materials fell out of use
}
