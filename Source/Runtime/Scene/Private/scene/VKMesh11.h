...
class VKIndirectBuffer11 final
{
  public:
    VKIndirectBuffer11(const std::unique_ptr<lvk::IContext> &ctx, size_t maxDrawCommands,
        ...
        : ctx_(ctx), drawCommands_(maxDrawCommands)
    {
        ...
        bufferIndirect_ =
            ctx->createBuffer({.usage = lvk::BufferUsageBits_Indirect | lvk::BufferUsageBits_Storage,
    ...
    void uploadIndirectBuffer()
    {
        ...
        ctx_->upload(bufferIndirect_, drawCommands_.data(), sizeof(VkDrawIndexedIndirectCommand) * numCommands,
                     sizeof(uint32_t));
        if (!drawData_.empty())
        {
    ...
    {
        ctx_->flushMappedMemory(bufferIndirect_, 0,
                                sizeof(uint32_t) + sizeof(DrawIndexedIndirectCommand) * numCommands);
...
class VkPipelineDeferred : public VKPipeline
{
  public:
                       ...
                       lvk::Holder<lvk::ShaderModuleHandle> &&frag = {}, bool positionOnly = false)
    {
        positionOnly_ = positionOnly;
        vert_ =
            vert.valid() ? std::move(vert) : loadShaderModule(ctx, "PTRenderer/src/gbuffer_opaque.vert");
        frag_ =
            frag.valid() ? std::move(frag) : loadShaderModule(ctx, "PTRenderer/src/gbuffer_opaque.frag");
...
class VKMesh11
{
  public:
    VKMesh11(const std::unique_ptr<lvk::IContext> &ctx, const MeshData &meshData, const Scene &scene,
        ...
        : ctx(ctx), numIndices_((uint32_t)meshData.indexData.size()), numMeshes_((uint32_t)meshData.meshes.size()),
          ...
          textureFiles_(meshData.textureFiles)
    {
        ...
        bufferPositions_ = ctx->createBuffer(
        ...
        bufferAttributes_ = ctx->createBuffer({.usage = lvk::BufferUsageBits_Vertex | lvk::BufferUsageBits_Storage,
                                              ...
                                              nullptr);
        bufferIndices_ =
            ctx->createBuffer({.usage = lvk::BufferUsageBits_Index | lvk::BufferUsageBits_AccelStructBuildInputReadOnly,
                               .storage = lvk::StorageType_Device,
              ...
              lvk::TextureHandle texSkyboxIrradiance = {}, const VKIndirectBuffer11 *indirectBuffer = nullptr) const
    {
        ...
        buf.cmdDrawIndexedIndirectCount(indirectBuffer->bufferIndirect_, sizeof(uint32_t),
                                        indirectBuffer->bufferIndirect_, 0, numMeshes_,
    ...
    DrawIndexedIndirectCommand *getDrawIndexedIndirectCommandPtr() const
    {
        return indirectBuffer_.getDrawIndexedIndirectCommandPtr();
    };
