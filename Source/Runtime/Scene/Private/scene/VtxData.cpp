...
bool isMeshDataValid(const char *fileName)
{
    ...
    if (header.magicValue != kMeshFileMagic)
    {
        return false;
    }

    if (fseek(f, sizeof(lvk::VertexInput) * 2, SEEK_CUR))
    {
        return false;
    }
    ...
    if (fseek(f, header.positionDataSize, SEEK_CUR))
    {
        return false;
    }

    if (fseek(f, header.attributeDataSize, SEEK_CUR))
    {
        return false;
    }
...
MeshFileHeader loadMeshData(const char *meshFile, MeshData &out)
{
    ...
    if (fread(&header, 1, sizeof(header), f) != sizeof(header))
    {
        LLOGL("Unable to read mesh file header.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    if (header.magicValue != kMeshFileMagic)
    {
        LLOGL("Unsupported mesh cache version in '%s'. Please regenerate the cache.\n", meshFile);
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    {
        LLOGL("Unable to read vertex streams description.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    if (fread(&out.positionOnlyStreams, 1, sizeof(out.positionOnlyStreams), f) != sizeof(out.positionOnlyStreams))
    {
        LLOGL("Unable to read position-only vertex streams description.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    {
        LLOGL("Could not read mesh descriptors.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    {
        LLOGL("Could not read bounding boxes.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    out.positionData.resize(header.positionDataSize);
    out.attributeData.resize(header.attributeDataSize);

    if (fread(out.indexData.data(), 1, header.indexDataSize, f) != header.indexDataSize)
    {
        LLOGL("Unable to read index data.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    {
        LLOGL("Unable to read vertex data.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }

    if (fread(out.positionData.data(), 1, header.positionDataSize, f) != header.positionDataSize)
    {
        LLOGL("Unable to read position data.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }

    if (fread(out.attributeData.data(), 1, header.attributeDataSize, f) != header.attributeDataSize)
    {
        LLOGL("Unable to read attribute data.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
...
void loadMeshDataMaterials(const char *fileName, MeshData &out)
{
    ...
    if (fread(&numMaterials, 1, sizeof(numMaterials), f) != sizeof(numMaterials))
    {
        LLOGL("Unable to read numMaterials.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    if (fread(&materialsSize, 1, sizeof(materialsSize), f) != sizeof(materialsSize))
    {
        LLOGL("Unable to read materialsSize.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    if (numMaterials * sizeof(Material) != materialsSize)
    {
        LLOGL("Corrupted material file '%s'.\n", fileName);
        assert(false);
        exit(EXIT_FAILURE);
    }
    ...
    {
        LLOGL("Unable to read material data.\n");
        assert(false);
        exit(EXIT_FAILURE);
    }
...
void saveMeshData(const char *fileName, const MeshData &m)
{
    ...
    if (!f)
    {
        LLOGL("Error opening file '%s' for writing.\n", fileName);
        assert(false);
        exit(EXIT_FAILURE);
    }

    const MeshFileHeader header = m.getMeshFileHeader();

    fwrite(&header, 1, sizeof(header), f);
    fwrite(&m.streams, 1, sizeof(m.streams), f);
    fwrite(&m.positionOnlyStreams, 1, sizeof(m.positionOnlyStreams), f);
    ...
    fwrite(m.positionData.data(), 1, header.positionDataSize, f);
    fwrite(m.attributeData.data(), 1, header.attributeDataSize, f);
...
void saveBoundingBoxes(const char *fileName, const std::vector<BoundingBox> &boxes)
{
    ...
    if (!f)
    {
        LLOGL("Error opening bounding boxes file '%s' for writing.\n", fileName);
        assert(false);
        exit(EXIT_FAILURE);
    }
...
void loadBoundingBoxes(const char *fileName, std::vector<BoundingBox> &boxes)
{
    ...
    if (!f)
    {
        LLOGL("Error opening bounding boxes file '%s'\n", fileName);
        assert(false);
        exit(EXIT_FAILURE);
    }
...
MeshFileHeader mergeMeshData(MeshData &m, const std::vector<MeshData *> md)
{
    ...
    if (!md.empty())
    {
        ...
        m.positionOnlyStreams = md[0]->positionOnlyStreams;
    }

    const uint32_t vertexSize = m.positionOnlyStreams.getVertexSize();
    ...
    for (const MeshData *i : md)
    {
        ...
        mergeVectors(m.positionData, i->positionData);
        mergeVectors(m.attributeData, i->attributeData);
        ...
        numTotalVertices += (uint32_t)i->positionData.size() / vertexSize;
    }

    return MeshFileHeader{
        .magicValue = kMeshFileMagic,
        ...
        .positionDataSize = static_cast<uint32_t>(m.positionData.size()),
        .attributeDataSize = static_cast<uint32_t>(m.attributeData.size()),
    };
}
