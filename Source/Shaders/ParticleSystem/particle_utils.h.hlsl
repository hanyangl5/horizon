...
#include "particle_packing.h.hlsl"
...
uint GetCurrentBufferSection(uint particleIdx)
{
	...
	uint currBufferOffset = ParticleCountsBuffer[0].x;

	while (currBufferOffset <= particleIdx && currBufferSection < 2)
	{
		currBufferSection++;
		currBufferOffset += ParticleCountsBuffer[0][currBufferSection];
	}
...
bool TrySwappingActivity(bool isActive, uint currBufferSection, uint nAttempts, uint particleIdx, uint particleCount, inout(uint) currBitfield, inout(ParticleData) particleData)
{
	...
	uint offset = ParticleSectionsIndices[currBufferSection * 2 + CURR_VALUE] - (isActive ? particleCount : 0);
	...
	while (isActive == bool(ParticleSetVisibility[setIndex*2+CURR_VALUE]) &&							// Make sure the received particle has the right state
		sign*int(currIdx) < sign*int(ParticleSectionsIndices[currBufferSection * 2 + CURR_VALUE]) &&	// Don't read from the wrong section
		attemptCount < nAttempts);																			// Only try a certain amount of times

	if (bool(ParticleSetVisibility[setIndex*2+CURR_VALUE]) != isActive)
	{
