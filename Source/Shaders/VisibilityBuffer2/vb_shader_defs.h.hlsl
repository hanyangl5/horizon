...
// Should be a multiple of the wavefront size
#define FILTER_BATCH_SIZE 256

// This defines the amount of triangles that will be processed in parallel by the
// compute shader in the rasterization process.
// Should be a multiple of the wavefront size
#define RASTERIZE_BATCH_SIZE 64

// FILTER_BATCH_COUNT limits the amount of triangle batches we can process on the GPU.
// It depends on the amoutnt of data we need to store in the constant buffers, since
// the max constant buffer size is 64KB -> sizeof(FilterBatchData) * 2048 = 64KB
#define FILTER_BATCH_COUNT 2048
...
#if !defined(ALPH_IS_LOW_BIT) && !defined(TRI_ID_LOW_BIT) && !defined(BATCH_ID_LOW_BIT)
#define ALPH_IS_LOW_BIT 31 // 1  bit  for Alpha
#define TRI_ID_LOW_BIT 23 // 8  bits for TriID
#define BATCH_ID_LOW_BIT  0 // 23 bits for BatchID

#define MATERIAL_ID_LOW_BIT  0 // 23 bits for MaterialID

#define ALPH_IS_MASK 0x80000000 // 1000 0000 0000 0000 0000 0000 0000 0000
#define TRI_ID_MASK 0x7F800000 // 0111 1111 1000 0000 0000 0000 0000 0000
#define BATCH_ID_MASK 0x007FFFFF // 0000 0000 0111 1111 1111 1111 1111 1111

#define MATERIAL_ID_MASK 0x007FFFFF // 0000 0000 0111 1111 1111 1111 1111 1111
#endif

#define GET_OUTPUT_VB_ID(triId, batchId) ((((triId) << TRI_ID_LOW_BIT) & TRI_ID_MASK) | (((batchId) << BATCH_ID_LOW_BIT) & BATCH_ID_MASK))
#define GET_ALPHA_OUTPUT_VB_ID(triId, batchId) ((1 << ALPH_IS_LOW_BIT) | (((triId) << TRI_ID_LOW_BIT) & TRI_ID_MASK) | (((batchId) << BATCH_ID_LOW_BIT) & BATCH_ID_MASK))

#define PACK_BATCH_DATA(geomSet, triId, materialId) (((geomSet) << ALPH_IS_LOW_BIT) | (((triId) << TRI_ID_LOW_BIT) & TRI_ID_MASK) | (((materialId) << MATERIAL_ID_LOW_BIT) & MATERIAL_ID_MASK))
...
#ifndef MAX_BATCHES
#define MAX_BATCHES 65536   // Maximum number of batches allowed in visibility buffer
#define BATCH_DISPATCH_ARGUMENTS_OFFSET 3
#define HW_RASTERIZED_BATCH_ARGUMENTS_OFFSET MAX_BATCHES / 2
#endif

// The following value defines the maximum amount of triangles we can draw on each indirect draw call. 
// The values of FILTER_BATCH_COUNT, FILTER_BATCH_SIZE and MAX_PRIMITIVES_PER_DRAW_INDIRECT determine how many invocations to 
// triangle_filtering are needed, one of the values will exhaust first the capacity of each filtering batch.
// 
// If MAX_PRIMITIVES_PER_DRAW_INDIRECT is smaller than FILTER_BATCH_COUNT * FILTER_BATCH_SIZE, then it means we'll be making more 
// triangle_filtering dispatches and none of them will use the max capacity of the shader. This is needed when bigger
// DrawID values are needed, DrawID needs more bits and PrimitiveID has less bits.
#ifndef MAX_PRIMITIVES_PER_DRAW_INDIRECT
#define MAX_PRIMITIVES_PER_DRAW_INDIRECT (1 + (BATCH_ID_MASK >> BATCH_ID_LOW_BIT)) // Maximum value that can be represented with # bits PrimitiveID
 #endif
...
#define GET_COMPACT_DRAW_INDEX(VIEW, DRAW_ID) (((NUM_CULLING_VIEWPORTS + (VIEW)) * MAX_DRAWS_INDIRECT) + (DRAW_ID))
