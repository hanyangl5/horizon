/*****************************************************************/ /**
                                                                     ...
                                                                     *         Provides SimpleMath-compatible API for Android platform
                                                                     *
...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        struct Vector2
        {
            ...
            Vector2& operator/=(float S) noexcept { float inv = 1.0f / S; x *= inv; y *= inv; return *this; }
            ...
            {
                float len = Length();
                if (len > 0.0f) { float inv = 1.0f / len; x *= inv; y *= inv; }
            }

            void Normalize(Vector2& result) const noexcept
            {
                float len = Length();
                if (len > 0.0f) { float inv = 1.0f / len; result.x = x * inv; result.y = y * inv; }
                else { result.x = 0.0f; result.y = 0.0f; }
            }
        ...
        struct Vector3
        {
            ...
            Vector3& operator/=(float S) noexcept { float inv = 1.0f / S; x *= inv; y *= inv; z *= inv; return *this; }
            ...
            Vector3 operator/(float S) const noexcept { float inv = 1.0f / S; return Vector3(x * inv, y * inv, z * inv); }
            ...
            {
                float len = Length();
                if (len > 0.0f) { float inv = 1.0f / len; x *= inv; y *= inv; z *= inv; }
            }

            void Normalize(Vector3& result) const noexcept
            {
                float len = Length();
                if (len > 0.0f) { float inv = 1.0f / len; result.x = x * inv; result.y = y * inv; result.z = z * inv; }
                else { result.x = 0.0f; result.y = 0.0f; result.z = 0.0f; }
            }
        ...
        struct Vector4
        {
            ...
            Vector4& operator/=(float S) noexcept { float inv = 1.0f / S; x *= inv; y *= inv; z *= inv; w *= inv; return *this; }
            ...
            {
                float len = Length();
                if (len > 0.0f) { float inv = 1.0f / len; result.x = x * inv; result.y = y * inv; result.z = z * inv; result.w = w * inv; }
                else { result.x = 0.0f; result.y = 0.0f; result.z = 0.0f; result.w = 0.0f; }
            }
        ...
        struct Matrix
        {
            ...
            Matrix& operator*=(const Matrix& M) noexcept { *this = *this * M; return *this; }
            ...
            static Matrix CreateLookAt(const Vector3& eye, const Vector3& target, const Vector3& up) noexcept
            {
                ...
                result.m[3][0] = -xaxis.Dot(eye); result.m[3][1] = -yaxis.Dot(eye); result.m[3][2] = -zaxis.Dot(eye); result.m[3][3] = 1.0f;
                return result;
            }
        ...
        inline const Matrix Matrix::Identity = []() {
            ...
            tm.m[0][0] = 1.0f; tm.m[1][1] = 1.0f; tm.m[2][2] = 1.0f; tm.m[3][3] = 1.0f;
            return tm;
        }();
    }
}
