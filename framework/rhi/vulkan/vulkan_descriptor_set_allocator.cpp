...
void VulkanDescriptorSetAllocator::CreateDescriptorSetLayout(VulkanPipeline *pipeline)
{
    ...
    for (const auto &[set_number, descriptors] : rsd.descriptors)
    {
        ...
        if (is_bindless)
        {
            ...
            // Policy (per your choice A): make Textures the only variable-count binding, and ensure it's the highest
            // binding. Other bindless bindings (e.g. samplers/buffers) are fixed-size.
            for (auto &b : bindings)
            {
            ...
            // Enforce: highest binding must be the texture binding (required because VARIABLE_DESCRIPTOR_COUNT must be
            // on it).
            if (!bindings.empty())
            {
                ...
                if (!last_is_texture)
                {
                    LOG_ERROR("Bindless set{}: expected highest binding to be a texture binding for variable "
                              "descriptor count, but got type={} (binding={}).",
...
VulkanDescriptorSet *VulkanDescriptorSetAllocator::GetBindlessDescriptorSet(VulkanPipeline *pipeline)
{
    ...
    u32 maxDescriptors =
        k_max_bindless_resources; // Maximum descriptors for the variable-sized binding (highest binding)
