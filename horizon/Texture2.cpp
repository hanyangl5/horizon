...
void Texture::loadFromBuffer(void* buffer, VkDeviceSize bufferSize, VkFormat format, uint32_t texWidth, uint32_t texHeight, Device* device, VkFilter filter, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
{
...
void Texture::createImageView(VkFormat format) {
	...
	vkCreateImageView(mDevice->get(), &viewInfo, nullptr, &mImageView);
}
