...
void createPipelineReflection(ShaderReflection *pReflection, uint32_t stageCount, PipelineReflection *pOutReflection) {
    ...
    if (pReflection == NULL) {
        ...
        LOG_ERROR("Parameter 'pReflection' is NULL.");
        return;
    }
    if (stageCount == 0) {
    ...
    if (pOutReflection == NULL) {
        LOG_ERROR("Parameter 'pOutShaderReflection' is NULL.");
        return;
    }

    // Sanity check to make sure we don't have repeated stages.
    ShaderStageFlags combinedShaderStages = (ShaderStageFlags)0;
    for (uint32_t i = 0; i < stageCount; ++i) {
        ...
        combinedShaderStages = (ShaderStageFlags)(combinedShaderStages | pReflection[i].mShaderStage);
    }
    ...
    for (uint32_t i = 0; i < stageCount; ++i) {
        ...
        if (pSrcRef->mShaderStage == ShaderStageFlags::SHADER_STAGE_VERTEX_SHADER) {
            vertexStageIndex = i;
        }
//#if !defined(METAL)
...
//        }
//#endif
        else if (pSrcRef->mShaderStage == ShaderStageFlags::SHADER_STAGE_PIXEL_SHADER) {
            pixelStageIndex = i;
        }
    ...
    if (resourceCount) {
        pResources = (ShaderResource *)calloc(resourceCount, sizeof(ShaderResource));
