...
#include <runtime/function/rhi/vulkan/VulkanShaderReflection.h>
...
PipelineLayoutDesc VulkanDescriptorSetManager::GetComputePipelineLayout(VulkanShaderProgram *cs) {
    ...
    vk_createShaderReflection((u8*)cs->m_spirv_code.data(), cs->m_spirv_code.size(),
                              ShaderStageFlags::SHADER_STAGE_COMPUTE_SHADER, &reflections);
    cs->p_reflection = new PipelineReflection;
    createPipelineReflection(&reflections, 1, cs->p_reflection);

    for (u32 i = 0; i < cs->p_reflection->mShaderResourceCount; i++) {

        cs->p_reflection->pShaderResources[i];
    }
    return {};
}
