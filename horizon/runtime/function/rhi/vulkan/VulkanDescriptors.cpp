...
namespace Horizon::RHI {
	...
	void VulkanDescriptor::AllocateDescriptors() noexcept
	{
		...
		if (m_bindless_descriptor_pool == VK_NULL_HANDLE) {
			...
			std::transform(m_bindless_descriptor_types.begin(), m_bindless_descriptor_types.end(),
				std::back_inserter(vk_types), [](DescriptorType type) {
					return ToVkDescriptorType(type);
				});

			std::vector<VkDescriptorPoolSize> pool_sizes{};

			std::transform(vk_types.begin(), vk_types.end(),
				std::back_inserter(pool_sizes), [](VkDescriptorType type) {
