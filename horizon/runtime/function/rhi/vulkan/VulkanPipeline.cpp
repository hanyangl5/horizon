...
void VulkanPipeline::CreateGraphicsPipeline() noexcept {
    ...
    // shader stage
    {
        ...
        graphics_pipeline_create_info.stageCount =
    ...
    // vertex input state
    {
        ...
        VkVertexInputBindingDescription
            input_bindings[VertexInputState::MAX_BINDING_COUNT] = {{0}};
        uint32_t input_attribute_count = 0;
        VkVertexInputAttributeDescription
            input_attributes[VertexInputState::MAX_ATTRIBUTE_COUNT] = {{0}};
        ...
        for (u32 i = 0; i < ci->vertex_input_state.attribute_count; ++i) {
            ...
            if (attrib->input_rate == VertexInputRate::VERTEX_ATTRIB_RATE_INSTANCE) {
                input_bindings[input_binding_count - 1].inputRate =
            ...
            input_bindings[input_binding_count - 1].stride +=
                GetStrideFromVertexAttributeDescription(
                    attrib->attrib_format, attrib->portion);

            input_attributes[input_attribute_count].location =
                attrib->location;
    ...
    // input assembly state
    {
        ...
        input_assembly_state_create_info.topology =
            ToVkPrimitiveTopology(ci->input_assembly_state.topology);
...

    {
        ...
        view_port_state_create_info.sType =
...

    {
        ...
        rasterization_state_create_info.cullMode =
            ToVkCullMode(ci->raserization_state.cull_mode);
        rasterization_state_create_info.frontFace =
            ToVkFrontFace(ci->raserization_state.front_face);
    ...
    // TODO: mulitsampling
    {
        ...
        multi_sample_state_create_info.sType =
    ...
    // depth stencil
    {
        ...
        depth_stencil_state_create_info.depthTestEnable =
            ci->depth_stencil_state.depth_test;
        depth_stencil_state_create_info.depthWriteEnable =
            ci->depth_stencil_state.depth_write;
        depth_stencil_state_create_info.depthCompareOp =
            ToVkCompareOp(ci->depth_stencil_state.depth_func);
...
void VulkanPipeline::CreateComputePipeline() noexcept {
    ...
    auto cs = reinterpret_cast<VulkanShaderProgram *>(
