...
static TextureDimension sSPIRV_TO_RESOURCE_DIM[SPIRV_DIM_COUNT] = {
    TEXTURE_DIM_UNDEFINED, TEXTURE_DIM_UNDEFINED, TEXTURE_DIM_1D,         TEXTURE_DIM_1D_ARRAY,
    TEXTURE_DIM_2D,        TEXTURE_DIM_2D_ARRAY,  TEXTURE_DIM_2DMS,       TEXTURE_DIM_2DMS_ARRAY,
    TEXTURE_DIM_3D,        TEXTURE_DIM_CUBE,      TEXTURE_DIM_CUBE_ARRAY,
};

bool filterResource(SPIRV_Resource *resource, ShaderStageFlags currentStage) {
    bool filter = false;
...
void vk_createShaderReflection(const uint8_t *shaderCode, uint32_t shaderSize, ShaderStageFlags shaderStage,
                               ShaderReflection *pOutReflection) {
    if (pOutReflection == NULL) {
        LOG_ERROR("Create Shader Refection failed. Invalid reflection output!");
    ...
    if (shaderStage == ShaderStageFlags::SHADER_STAGE_COMPUTE_SHADER) {
        ReflectComputeShaderWorkGroupSize(&cc, &pOutReflection->mNumThreadsPerGroup[0],
                                          &pOutReflection->mNumThreadsPerGroup[1],
                                          &pOutReflection->mNumThreadsPerGroup[2]);
    } else if (shaderStage == ShaderStageFlags::SHADER_STAGE_TESS_SHADER) {
    ...
    char *namePool = NULL;
    if (namePoolSize)
        namePool = (char *)calloc(namePoolSize, 1);
    ...
    if (shaderStage == ShaderStageFlags::SHADER_STAGE_VERTEX_SHADER && vertexInputCount > 0) {
        ...
        for (uint32_t i = 0; i < cc.ShaderResourceCount; ++i) {
            ...
            // filter out what we don't use
            if (!filterResource(resource, shaderStage) &&
                resource->type == SPIRV_Resource_Type::SPIRV_TYPE_STAGE_INPUTS) {
    ...
    if (resourceCount) {
        indexRemap = (uint32_t *)malloc(sizeof(uint32_t) * cc.ShaderResourceCount);
