...
namespace Horizon {
Camera::Camera(Math::float3 eye, Math::float3 at, Math::float3 up) noexcept
...
void Camera::SetPerspectiveProjectionMatrix(f32 fov, f32 aspect_ratio,
                                            f32 nearPlane,
...
void Camera::UpdateViewMatrix() noexcept {
    ...
    m_right = Math::Normalize(Math::Cross(
        m_forward,
        Math::float3(0.0, 1.0,
                     0.0))); // normalize the vectors, because their length gets
