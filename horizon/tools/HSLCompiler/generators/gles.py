...
from utils import isArray, getArrayLen, resolveName, getArrayBaseName, hsl_assert
...
def is_sampler(hsl_declaration):
    return 'SamplerState' in hsl_declaration[0]
...
def is_buffer(hsl_declaration):
    # TODO: endswith better here?
    return 'Buffer' in hsl_declaration[0]


# writeable textures get translated to glsl images
def is_rw_texture(hsl_declaration):
    dtype = hsl_declaration[0]
    writeable_types = [
...
def declare_rw_texture(hsl_declaration):
    tex_type, tex_name, freq, _, binding = hsl_declaration
    access = ''
...
def gles(hsl, dst):
    ...
    shader = getShader(hsl, dst, line_directives=False)
    ...
    def declare_buffer(hsl_declaration):
        ...
        buffer_type, name, _, _, _ = hsl_declaration
    ...
    for line in shader.lines:
        ...
        if '#ifdef NO_HSL_DEFINITIONS' in line:
            skip_line = True
        ...
        if 'CBUFFER' in line:
            hsl_assert(parsing_ubuffer == None, message='Inconsistent cbuffer declaration: \"' + line + '\"')
            parsing_ubuffer, _, _, _= getMacro(line)
            continue
        if 'PUSH_CONSTANT' in line:
            hsl_assert(parsing_ubuffer == None, message='Inconsistent push_constant declaration: \"' + line + '\"')
        ...
        if line.strip().startswith('RES('):
            ...
            print(resource_decl)
            hsl_assert(len(resource_decl) == 5, hsl, message='invalid Res declaration: \''+line+'\'')
