...
// helper functions
void ReflectBoundResources(spirv_cross::Compiler *pCompiler,
                           const spirv_cross::SmallVector<spirv_cross::Resource> &allResources,
                           const std::unordered_set<spirv_cross::VariableID> &usedResouces, SPIRV_Resource *resources,
                           uint32_t *current_resource, SPIRV_Resource_Type spriv_type) {
    for (size_t i = 0; i < allResources.size(); ++i) {
        ...
        // textureBuffer is considered as separate image  with dimension buffer in SpirV but they require a buffer
        // descriptor of type uniform texel buffer imageBuffer is considered as storage image with dimension buffer in
        // SpirV but they require a buffer descriptor of type storage texel buffer
...
void ReflectShaderResources(CrossCompiler *pCompiler) {
    ...
    for (size_t i = 0; i < allResources.stage_inputs.size(); ++i) {
        ...
        resource.binding = compiler->get_decoration(
            resource.SPIRV_code.id, spv::DecorationLocation); // location is the binding point for inputs
    ...
    ReflectBoundResources(compiler, allResources.uniform_buffers, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_UNIFORM_BUFFERS);
    ReflectBoundResources(compiler, allResources.storage_buffers, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_STORAGE_BUFFERS);
    ReflectBoundResources(compiler, allResources.storage_images, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_STORAGE_IMAGES);
    ReflectBoundResources(compiler, allResources.separate_images, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_IMAGES);
    ReflectBoundResources(compiler, allResources.separate_samplers, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_SAMPLERS);
    ReflectBoundResources(compiler, allResources.sampled_images, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_COMBINED_SAMPLERS);
    ReflectBoundResources(compiler, allResources.subpass_inputs, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_SUBPASS_INPUTS);
    ReflectBoundResources(compiler, allResources.subpass_inputs, usedResouces, resources, &current_resource,
                          SPIRV_TYPE_SUBPASS_INPUTS);
    ReflectBoundResources(compiler, allResources.acceleration_structures, usedResouces, resources, &current_resource,
    ...
    for (uint32_t i = 0; i < pCompiler->ShaderResourceCount; ++i) {
        ...
        if (resource.type == SPIRV_TYPE_UNIFORM_BUFFERS || resource.type == SPIRV_TYPE_PUSH_CONSTANT) {
            ...
            for (uint32_t j = 0; j < (uint32_t)type.member_types.size(); ++j) {
                ...
                variable.offset =
                    compiler->get_member_decoration(resource.SPIRV_code.base_type_id, j, spv::DecorationOffset);
            ...
            spirv_cross::SmallVector<spirv_cross::BufferRange> range =
                compiler->get_active_buffer_ranges(resource.SPIRV_code.id);
...
void ReflectComputeShaderWorkGroupSize(CrossCompiler *pCompiler, uint32_t *pSizeX, uint32_t *pSizeY, uint32_t *pSizeZ) {
    spirv_cross::Compiler *compiler = (spirv_cross::Compiler *)pCompiler->pCompiler;
    spirv_cross::SPIREntryPoint *pEntryPoint =
        &compiler->get_entry_point(pCompiler->pEntryPoint, compiler->get_execution_model());
