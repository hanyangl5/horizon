...
struct DiffuseIrradianceSH3 { float3 sh[9]; };

float3 Irradiance_SphericalHarmonics(DiffuseIrradianceSH3 ibl, float3 normal)
{
    return max(
        ibl.sh[0] + ibl.sh[1] * normal.y + ibl.sh[2] * normal.z + ibl.sh[3] * normal.x + ibl.sh[4] * (normal.y * normal.x) +
        ibl.sh[5] * (normal.y * normal.z) + ibl.sh[6] * (3.0 * normal.z * normal.z - 1.0) +
        ibl.sh[7] * (normal.z * normal.x) + ibl.sh[8] * (normal.x * normal.x - normal.y * normal.y),
        0.0);
}

float3 fresnelSchlickRoughness(float cosTheta, float3 F0, float roughness)
{
    return F0 + (max(float3(1.0 - roughness, 1.0 - roughness, 1.0 - roughness), F0) - F0) * Pow5(clamp(1.0 - cosTheta, 0.0, 1.0));
}
