...
{
    float3 curr_color = curr_color_tex.Load(threadID.xy).xyz;
    float2 motion_vector = mv_tex.Load(threadID.xy).xy;
    int2 prevCoord = (int2)threadID.xy - (int2)(motion_vector * kResolution);
    float3 prev_color = prev_color_tex.Load(prevCoord).xyz;

    float3 c0 = curr_color_tex.Load(threadID.xy + int2(1, 0)).xyz;
    float3 c1 = curr_color_tex.Load(threadID.xy + int2(-1, 0)).xyz;
    float3 c2 = curr_color_tex.Load(threadID.xy + int2(0, 1)).xyz;
    float3 c3 = curr_color_tex.Load(threadID.xy + int2(0, -1)).xyz;
