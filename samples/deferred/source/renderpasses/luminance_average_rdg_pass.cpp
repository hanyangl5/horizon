...
LuminanceAverageRDGPass::LuminanceAverageRDGPass(RHI *rhi)
    : RDGPass("Luminance Average Pass", rhi), m_rhi(rhi)
{
    m_luminance_average_cs = CreateShader(ShaderType::COMPUTE_SHADER, shader_dir / "luminance_average.comp.hlsl", "main");
...
void LuminanceAverageRDGPass::Execute(CommandList *cl, Horizon::Backend::FrameGraphBuilder &builder)
{
    ...
    {
        m_luminance_average_pipeline->SetResource(m_luminance_histogram_pass->GetConstantsBuffer(), "LuminanceHistogramConstants_cb");
    }
