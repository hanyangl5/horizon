...
inline std::pmr::memory_resource *GetGlobalAllocator() { return global_memory_resource; };
...
//_NODISCARD std::unique_ptr<T> MakeUnique(std::pmr::memory_resource &allocator, Args &&...args) { // make a unique_ptr
...
//template <typename T, typename... Args, std::enable_if_t<!std::is_array_v<T>, int> = 0>
//_NODISCARD std::unique_ptr<T> MakeUnique(Args &&...args) { // make a unique_ptr
//    return std::make_unique<T, Args...>(*Horizon::Memory::global_memory_resource, std::forward<Args>(args)...);
//}
...
//_NODISCARD std::unique_ptr<T> MakeUnique(std::pmr::memory_resource &allocator, const size_t _Size) { // make a unique_ptr
//     using _Elem = std::remove_extent_t<T>;
//     void *memory = allocator.allocate(sizeof(T) * _Size, alignof(std::max_align_t));
//     return std::unique_ptr<T>(new (memory) _Elem[_Size]());
// }
