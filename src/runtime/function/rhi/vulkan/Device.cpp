...
VkPhysicalDevice Device::getPhysicalDevice() const noexcept { return m_physical_devices[m_physical_device_index]; }

VkDevice Device::Get() const noexcept { return m_device; }

VkQueue Device::getGraphicQueue() const noexcept { return m_graphics_queue; }

VkQueue Device::getPresnetQueue() const noexcept { return m_present_queue; }
...
void Device::setPhysicalDevice(u32 device_index) { m_physical_device_index = device_index; }
...
void Device::createDevice(const ValidationLayer &validation_layers) {
    ...
    for (u32 queue_family : unique_queue_families) {
        device_queue_create_info.emplace_back(VkDeviceQueueCreateInfo{VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
                                                                      nullptr, 0, queue_family, 1, &queue_priority});
    }
    ...
    CHECK_VK_RESULT(
        vkCreateDevice(m_physical_devices[m_physical_device_index], &device_create_info, nullptr, &m_device));
...
QueueFamilyIndices Device::getQueueFamilyIndices() const noexcept { return m_queue_family_indices; }
