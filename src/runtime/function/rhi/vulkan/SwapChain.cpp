...
namespace Horizon {
SwapChain::SwapChain(RenderContext &render_context, std::shared_ptr<Device> device, std::shared_ptr<Surface> surface)
    : m_render_context(render_context), m_device(device), m_surface(surface) {
    createSwapChain();
...
VkSwapchainKHR SwapChain::Get() const noexcept { return m_swap_chain; }

std::vector<VkImageView> SwapChain::getImageViews() const noexcept { return imageViews; }

VkImageView SwapChain::getImageView(u32 i) const noexcept { return imageViews[i]; }

VkFormat SwapChain::getImageFormat() const noexcept { return mImageFormat; }
...
void SwapChain::createSwapChain() {
    ...
    swap_chain_create_info.imageUsage =
        ...
        VK_IMAGE_USAGE_TRANSFER_DST_BIT; //  the image can be used as the source of a transfer command.
    ...
    if (indices.getGraphics() != indices.getPresent()) {
        ...
        swap_chain_create_info.imageSharingMode =
            VK_SHARING_MODE_CONCURRENT; // An image is owned by one queue family at a time and ownership must be explicitly transferred before using it in another queue family. This option offers the best performance.
        ...
        swap_chain_create_info.imageSharingMode =
            VK_SHARING_MODE_EXCLUSIVE; // mages can be used across multiple queue families without explicit ownership transfers.
...
u32 SwapChain::chooseMinImageCount(VkSurfaceCapabilitiesKHR capabilities) {
    ...
    if (capabilities.maxImageCount > 0 && imag_count > capabilities.maxImageCount) {
