...
Math::mat4 Camera::GetProjectionMatrix() const noexcept { return m_projection; }

// Math::vec3 Camera::GetFov() const noexcept { return Math::vec3(); }

Math::vec2 Camera::GetNearFarPlane() const noexcept { return {m_near_plane, m_far_plane}; }
...
void Camera::Rotate(f32 xoffset, f32 yoffset) noexcept {
    m_yaw += xoffset;
    m_pitch -= yoffset; // TODO(hyl5): unify axis in different API
...
Math::mat4 Camera::GetInvViewProjectionMatrix() const noexcept { return Math::inverse(m_projection * m_view); }
Math::vec3 Camera::GetForwardDir() const noexcept { return m_forward; }
Math::mat4 Camera::GetViewMatrix() const noexcept { return m_view; }
Math::vec3 Camera::GetPosition() const noexcept { return m_eye; }
} // namespace Horizon
