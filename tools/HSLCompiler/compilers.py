...
import subprocess
import os
from utils import Languages, hsl_assert

hsl_basepath = os.path.dirname(__file__)

_config = {
    Languages.DIRECT3D11: ('HSL_COMPILER_FXC', 'fxc.exe'),
    Languages.DIRECT3D12: ('HSL_COMPILER_DXC', 'dxc.exe'),
    Languages.VULKAN:     ('VULKAN_SDK', 'Bin/glslangValidator.exe'),
    Languages.METAL:      ('HSL_COMPILER_METAL', 'metal.exe'),
...
def vulkan(src, dst):
    ...
    params = ['-V', src, '-o', dst, '-I'+hsl_basepath]
    # params = ['-DHSL_GLSL', '-DSAMPLE_COUNT 2', '-V', src, '-o', dst, '-I'+hsl_basepath]
    # params = ['-DHSL_GLSL', '-DSAMPLE_COUNT 2', src, '-I'+hsl_basepath, '-E']
    if '.frag' in src:
        params += ['-S', 'frag']
    if '.vert' in src:
        params += ['-S', 'vert']
    ...
    hsl_assert(status == 0, src, message=output)
    return status
...
def metal(src, dst):
    ...
    # params = ['-D','HSL_METAL', '-dD', '-C', '-E', '-o', dst, tmp]
    # params = ['-D','HSL_METAL', '-dD', '-S', '-o', dst, tmp]
    # params = ['-dD', '-I', hsl_basepath, '-o', dst, tmp]

    params = ['-I', hsl_basepath]
    params += ['-dD', '-o', dst, src]
...
def d3d11(src, dst):
    ...
    params += ['/I', hsl_basepath, '/Fo', dst, src]
    # params = []
    # params += ['/I', hsl_basepath, '/P', dst, src]
...
def d3d12(src, dst):
    ...
    if '.tese' in src:
        params += ['/T', 'ds_6_0']
    # params += ['/I', hsl_basepath, '/DHSL_D3D12', '/Fo', dst, src]
    params += ['/I', hsl_basepath, '/Fo', dst, src]
...
def orbis(src, dst):
    ...
    params += ['-I'+hsl_basepath, '-o', dst + '.sb', src]
    # params += ['-I'+hsl_basepath, '-o', dst + '.sb', src, '-E']
    # params += ['-I'+hsl_basepath, src, '-E']
...
def prospero(src, dst):
    ...
    params += ['-I'+hsl_basepath, '-o', dst, src]
...
def xbox(src, dst):
    ...
    if '.tese' in src:
        params += ['/T', 'ds_6_0']
    params += ['/I', hsl_basepath, '/D__XBOX_DISABLE_PRECOMPILE', '/Fo', dst, src]
