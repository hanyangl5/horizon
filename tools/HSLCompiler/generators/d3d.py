...
from utils import Stages, getHeader, getShader, getMacro, genFnCall, hsl_assert, get_whitespace
from utils import isArray, getArrayLen, getArrayBaseName, getMacroName, DescriptorSets, is_groupshared_decl
import os, sys, importlib, re
from shutil import copyfile

def pssl(hsl, dst, rootSignature=None):
    return d3d(hsl, dst, pssl=True, d3d12=False, rootSignature=rootSignature)

def prospero(hsl, dst):
    return d3d(hsl, dst, pssl=True, prospero=True)

def xbox(hsl, dst, rootSignature=None):
    return d3d(hsl, dst, xbox=True, d3d12=True, rootSignature=rootSignature)

def d3d12(hsl, dst):
    return d3d(hsl, dst, d3d12=True)

def scarlett(hsl, dst, rootSignature=None):
    return xbox(hsl, dst, rootSignature)

def d3d(hsl, dst, pssl=False, prospero=False, xbox=False, rootSignature=None, d3d12=False):
    ...
    shader_src += ['#line 1 \"'+hsl.replace(os.sep, '/')+'\"\n']
    line_index = 0
    ...
    for line in shader.lines:
        ...
        if 'TESS_VS_SHADER(' in line and prospero:
            ...
            vs_hsl_path = os.path.join(os.path.dirname(hsl), vs_filename)
            ls_vs_filename = 'ls_'+vs_filename.replace('.hsl', '')
            vs_pssl = os.path.join(os.path.dirname(dst), ls_vs_filename)
            d3d(vs_hsl_path, vs_pssl, pssl=True, prospero=True)
            shader_src += [
                '#undef VS_MAIN\n',
