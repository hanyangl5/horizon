...
float length(int2 x) { return length(float2(x)); }
...
bool allGreaterThan(const float4 a, const float4 b) { return all(a > b); }
...
float degrees(float radians) { return (radians * 3.14159265359 / 180.0); }

float radians(float degrees) { return (degrees * 180.0 / 3.14159265359); }
...
inline float4x3 setCol(inout(float4x3) M, in(float3)col, const uint i) {
    M[i] = col;
    return M;
}
inline float4x2 setCol(inout(float4x2) M, in(float2)col, const uint i) {
    M[i] = col;
    return M;
}

inline float3x4 setCol(inout(float3x4) M, in(float4)col, const uint i) {
    M[i] = col;
    return M;
}
inline float3x3 setCol(inout(float3x3) M, in(float3)col, const uint i) {
    M[i] = col;
    return M;
}
inline float3x2 setCol(inout(float3x2) M, in(float2)col, const uint i) {
    M[i] = col;
    return M;
}

inline float2x4 setCol(inout(float2x4) M, in(float4)col, const uint i) {
    M[i] = col;
    return M;
}
inline float2x3 setCol(inout(float2x3) M, in(float3)col, const uint i) {
    M[i] = col;
    return M;
}
inline float2x2 setCol(inout(float2x2) M, in(float2)col, const uint i) {
    M[i] = col;
    return M;
}
...
inline float4x4 setRow(inout(float4x4) M, in(float4)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    M[2][i] = row[2];
    M[3][i] = row[3];
...
inline float4x3 setRow(inout(float4x3) M, in(float4)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    M[2][i] = row[2];
    M[3][i] = row[3];
...
inline float4x2 setRow(inout(float4x2) M, in(float4)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    M[2][i] = row[2];
    M[3][i] = row[3];
...
inline float3x4 setRow(inout(float3x4) M, in(float3)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    M[2][i] = row[2];
    return M;
}
inline float3x3 setRow(inout(float3x3) M, in(float3)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    M[2][i] = row[2];
    return M;
}
inline float3x2 setRow(inout(float3x2) M, in(float3)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    M[2][i] = row[2];
    return M;
...
inline float2x4 setRow(inout(float2x4) M, in(float2)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    return M;
}
inline float2x3 setRow(inout(float2x3) M, in(float2)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    return M;
}
inline float2x2 setRow(inout(float2x2) M, in(float2)row, const uint i) {
    M[0][i] = row[0];
    M[1][i] = row[1];
    return M;
}
...
#define AtomicStore(DEST, VALUE) atomic_store_explicit(&DEST, VALUE, memory_order_relaxed)
#define AtomicLoad(SRC) atomic_load_explicit(&SRC, memory_order_relaxed)
...
#define AtomicMin3D(DST, COORD, VALUE, ORIGINAL_VALUE)                                                                 \
    {                                                                                                                  \
        (ORIGINAL_VALUE) =                                                                                             \
            atomic_fetch_min_explicit(((device atomic_uint *)&(DST)[uint(COORD)]), (VALUE), memory_order_relaxed);     \
    }
...
template <typename T, metal::access A> vec<T, 4> _LoadTex3D(texture2d_array<T, A> tex, uint3 p, uint lod) {
    return tex.read(p, lod);
}

template <typename T, metal::access A> vec<T, 4> _LoadTex3D(texture3d<T, A> tex, uint3 p, uint lod) {
    return tex.read(p, lod);
}
...
template <typename T, metal::access A> int2 GetDimensions(const texture2d<T, A> t, sampler s) {
    return int2(t.get_width(), t.get_height());
}
template <typename T, metal::access A> int2 GetDimensions(const texturecube<T, A> t, sampler s) {
    return int2(t.get_width(), t.get_height());
}

template <typename T, metal::access A> int2 GetDimensions(const texture2d<T, A> t, uint _NO_SAMPLER) {
    return int2(t.get_width(), t.get_height());
}
template <typename T, metal::access A> int2 GetDimensions(const texturecube<T, A> t, uint _NO_SAMPLER) {
    return int2(t.get_width(), t.get_height());
}
...
#define LoadByte4(BYTE_BUFFER, ADDRESS)                                                                                \
    uint4(BYTE_BUFFER[((ADDRESS) >> 2) + 0], BYTE_BUFFER[((ADDRESS) >> 2) + 1], BYTE_BUFFER[((ADDRESS) >> 2) + 2],     \
...
template <typename T, metal::access A> void _Write3D(texture2d<T, A> tex, uint3 p, vec<T, 4> v) { tex.write(v, p); }

template <typename T, metal::access A> void _Write3D(texture2d_array<T, A> tex, uint3 p, vec<T, 4> v) {
    tex.write(v, uint2(p.xy), uint(p.z));
}

template <typename T, metal::access A> void _Write3D(texture3d<T, A> tex, uint3 p, vec<T, 4> v) { tex.write(v, p); }
...
inline ballot_t WaveActiveBallot(bool expr) {
    ...
    return uint4((uint)(((simd_vote::vote_t)activeLaneMask) & 0xFFFFFFFF),
                 (uint)(((simd_vote::vote_t)activeLaneMask >> 32) & 0xFFFFFFFF), uint2(0));
}
...
// simd_lane_id is made available inside MetalShader by hsl using ENABLE_WAVEOPS()
...
inline uint _WavePrefixCountBits(bool expr, uint simd_lane_id) {
    ...
    uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, min(simd_lane_id, 32u)),
                       extract_bits(0xFFFFFFFF, 0, (uint)max((int)simd_lane_id - 32, 0)), uint2(0));
